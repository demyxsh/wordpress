...
# Get versions
DEMYX_ALPINE_VERSION="$(docker exec demyx_wp cat /etc/os-release | grep VERSION_ID | cut -c 12- | sed -e 's/\r//g')"
DEMYX_PHP_VERSION="$(docker exec demyx_wp php -v | grep cli | awk -F '[ ]' '{print $2}' | sed -e 's/\r//g')"
